...
> **ステータス**: Phase A+B+C+D+E+F Complete (32/32 tasks全完了)
> **対象Phase**: Phase 1
...
- [x] **TASK-A02**: FastAPIエントリポイント + CORS + 設定管理
...
- [x] **TASK-A03**: PostgreSQL スキーマ + SQLAlchemy モデル
...
- [x] **TASK-A05**: LLMクライアント抽象化 + リトライラッパー
...
- [x] **TASK-B02a**: BPDフォーマットSpike（設計前提検証） ✅
...
- [x] **TASK-B04**: Neo4jローダー + 4フェーズバルクロード ✅
...
- [x] **TASK-B06a**: Hybrid Search Spike（Neo4j検索基盤検証） ✅
  ...
  - 要件: REQ-MAP-003, 006
  - 依存: TASK-B04
  - ファイル: `backend/scripts/spike_hybrid_search.py`
  - 優先度: P0
  - 複雑度: S（所要 1〜2時間）
  - 検証結果: (1) 日本語fulltext全5クエリでヒット ✓, (2) 2段階結合Cypher正常動作 ✓, (3) final_score 0-1範囲 ✓, (4) keywords追加不要（現行index十分）
  - ステータス: Completed

- [x] **TASK-B06**: Hybrid検索サービス実装 ✅
...
- [x] **TASK-C02**: AnalyzeRequirementノード ✅
  ...
  - 優先度: P0
  - 複雑度: M
  - 検証: LLMモックでキーワード/ドメイン/意図が正しく返されるpytest
  - ステータス: Completed

- [x] **TASK-C03**: GenerateQueryノード ✅
  ...
  - 優先度: P0
  - 複雑度: M
  - 検証: 初回クエリ生成+リトライ時search_history参照をpytestで検証
  - ステータス: Completed

- [x] **TASK-C04**: HybridSearchノード（LangGraphノード統合） ✅
...
- [x] **TASK-C05**: EvaluateResultsノード ✅
  ...
  - 優先度: P0
  - 複雑度: M
  - 検証: sufficient/insufficient判定 + retry_count更新 + search_history追記をpytestで検証
  - ステータス: Completed

- [x] **TASK-C06**: TraverseGraphノード ✅
  ...
  - 優先度: P0
  - 複雑度: M
  - 検証: モックNeo4jで関連ノード取得+ModuleOverviewコンテキスト生成をpytestで検証
  - ステータス: Completed

- [x] **TASK-C07**: FinalJudgmentノード ✅
  ...
  - 優先度: P0
  - 複雑度: L
  - 検証: confidence_score算出(0.4*search+0.6*llm)、temperature=0設定、Scope Item ID引用をpytestで検証
  - ステータス: Completed

- [x] **TASK-C08**: GenerateProposalTextノード ✅
  ...
  - 優先度: P0
  - 複雑度: M
  - 検証: proposal_text生成 + temperature=0.3設定 + completed_atタイムスタンプをpytestで検証
  - ステータス: Completed

- [x] **TASK-C09**: LangGraphワークフロー組み立て ✅
  ...
  - 優先度: P0
  - 複雑度: M
  - 検証: should_retry_search条件分岐(4パターン) + build_mapping_graph compile成功 + 全7ノード登録確認をpytestで検証
  - ステータス: Completed

- [x] **TASK-C10**: バッチプロセッサ + SSEキュー ✅
...
- [x] **TASK-D01**: 案件管理API（案件作成 + Excel取込） ✅
...
- [x] **TASK-D02**: マッピングAPI（開始 + SSE + 結果取得） ✅
...
- [x] **TASK-E01**: Next.jsプロジェクトセットアップ ✅
  ...
  - 優先度: P1
  - 複雑度: M
  - 検証: `pnpm build` 成功、`/` にダッシュボード骨格が表示される ✅
  - ステータス: Completed

- [x] **TASK-E02**: Excelアップロード + カラムマッピングUI ✅
...
- [x] **TASK-F01**: PoC用ナレッジデータ準備 ✅
  ...
  - 優先度: P1
  - 複雑度: L
  - 検証: Neo4jに565 ScopeItemノード + 1,036 RELATED_TO + 4 ModuleOverview + 23 COVERS。全ノードにEmbeddingあり ✅
  - ステータス: Completed

- [x] **TASK-F02**: E2E統合テスト ✅
  - 詳細: モックLangGraphで全フロー（Excel UP→案件作成→マッピング開始→SSE受信→結果取得）を通しテスト。10件要件で動作確認
  - 要件: （横断）
  - 依存: TASK-D02, TASK-E05, TASK-F01
  - ファイル: `backend/tests/e2e/test_full_flow.py`
  - 優先度: P1
  - 複雑度: L
  - 検証: 10件の要件が全てstatus=completedになり、judgment_level/confidence/proposal_textが全て非空 ✅ (7テスト)
  - ステータス: Completed

- [x] **TASK-F03**: PoC実行（フル案件マッピング） ✅
  - 詳細: 仕掛かり案件のRFP Excel（100-300要件）をフル投入→マッピング実行。処理時間・エラー率・LLMコスト計測。結果をレビューし、判定品質を人間チェック
  - 要件: NFR-MAP-001, 002
  - 依存: TASK-F01, TASK-F02
  - ファイル: `backend/scripts/run_poc.py`
  - 優先度: P1
  - 複雑度: L
  - 検証: Safie RFP 77要件でPoC実行。チューニング前: 77/77完了, 0エラー, 937.9秒(12.2秒/件), Low10件(13%)。チューニング後(28件サンプル): Low0件, 検索スコア0.0→0%, High75% ✅
  - ステータス: Completed

- [x] **TASK-F04**: 精度評価スクリプト実装 ✅
...
- [x] **TASK-F05**: チューニング・パラメータ最適化 ✅
  - 詳細: PoC結果に基づき3つの改善を実施: (1) Neo4j fulltextインデックスにkeywordsフィールド追加, (2) クエリ生成プロンプトからBoolean演算子を排除, (3) Hybrid検索0件時のvector-onlyフォールバック追加
  - 要件: NFR-MAP-003
  - 依存: TASK-F03, TASK-F04
  - ファイル: `backend/scripts/setup_neo4j_indexes.py`, `backend/app/services/mapping/nodes/generate_query.py`, `backend/app/services/knowledge/search.py`
  - 優先度: P2
  - 複雑度: L
  - 検証: 同一28要件比較 — 検索スコア0.0: 46.4%→0%, Low確信度: 8件→0件, High確信度: 10件→21件。130テスト全パス ✅
  - ステータス: Completed
...
### テスト状況
- Backend: **130テスト** 全パス（pytest）
- Frontend: **pnpm build** 成功（4ルート: /, /cases/new, /cases/[id]/mapping, /_not-found）

### PoC実行結果
- **F01**: 565 BPDセット + 4 PDF → Neo4j投入完了（565 ScopeItem, 1,036 RELATED_TO, 4 ModuleOverview）
- **F03**: Safie RFP 77要件マッピング → 77/77完了, エラー0, 937.9秒
- **F05**: チューニング3項目実施 → 検索スコア0.0が46.4%→0%に改善, Low確信度が8件→0件に
- **注意**: Anthropic APIクレジット残高不足のため、チューニング後のフル77件再実行は未完了（28件で検証済み）
